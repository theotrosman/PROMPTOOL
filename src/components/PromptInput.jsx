...
const getTimerConfig = (difficulty = 'Media') => {
  const normalized = normalizeDifficulty(difficulty)

  if (normalized.includes('facil')) {
    return { recommendedSeconds: 85, targetWords: 14, graceSeconds: 20 }
  }
  if (normalized.includes('dificil')) {
    return { recommendedSeconds: 180, targetWords: 30, graceSeconds: 30 }
  }
  return { recommendedSeconds: 130, targetWords: 22, graceSeconds: 25 }
}
...
const PromptInput = ({ promptUsuario, setPromptUsuario, onSubmit, isLoading, difficulty, onTimingChange }) => {
  const [startedAt, setStartedAt] = useState(null)
  const [elapsedSeconds, setElapsedSeconds] = useState(0)
  const timerConfig = useMemo(() => getTimerConfig(difficulty), [difficulty])
  ...
  return (
    <form className="space-y-4" onSubmit={onSubmit}>
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
        <span className="rounded-full bg-slate-100 px-3 py-1">Dificultad: {difficulty}</span>
        <span className="rounded-full bg-slate-100 px-3 py-1">Palabras: {wordsCount}</span>
        <span className="rounded-full bg-slate-100 px-3 py-1">Objetivo: {timerConfig.targetWords}+</span>
        <span className={`rounded-full px-3 py-1 ${timeBadgeClass}`}>Tiempo sugerido: {formatTime(remainingSeconds)}</span>
      </div>
